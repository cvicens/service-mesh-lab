...
of the command before execution.

If you hover over the command block on the rigth upper corner you'll find a clip board 
icon, click on it to copy the contents.
...
pre-installed, including the `oc` CLI. 

If `Terminal` is enabled, on the top navigation bar click on it, log in with your 
credentials and run this command.
