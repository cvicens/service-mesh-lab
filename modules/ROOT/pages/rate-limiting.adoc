...
spec:
  dimensions:
    source: request.headers["x-forwarded-for",subs="verbatim,attributes"] | "unknown"
    destination: destination.labels["app",subs="verbatim,attributes"] | destination.service.name | "unknown"
    destinationVersion: destination.labels["version",subs="verbatim,attributes"] | "unknown"
---
apiVersion: config.istio.io/v1alpha2
kind: QuotaSpec
metadata:
...
spec:
  ...
  # match: match(request.headers["cookie",subs="verbatim,attributes"], "user=*") == false
  actions:
